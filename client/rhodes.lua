...
Citizen.CreateThread(function() --Rhodes starting location
    while true do
		Citizen.Wait(0)
		local playerPed = PlayerPedId()
		local coords = GetEntityCoords(playerPed)
		local betweencoords = GetDistanceBetweenCoords(coords,1589.78, -1838.80, 52.4, true)
		if betweencoords < 2.0 and speaked == false then
			...
			if IsControlJustReleased(0, 0xC7B5340A) then
				speaked = true
                started = true    
				TriggerServerEvent("fists_robbery:rhodes", function() 
			end)
			end
		end
	end
